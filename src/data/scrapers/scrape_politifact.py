...
from data.scrapers.scrape_factcheck import scrape_factcheck_article
from data.scrapers.scrape_fullfact import scrape_fullfact_article
...
def scrape_article(url: str) -> Dict:
    try:
        if "fullfact.org" in url:
            return scrape_fullfact_article(url)
        elif "politifact.com" in url:
            return scrape_politifact_article(url)
        elif "factcheck.org" in url:
            return scrape_factcheck_article(url)
        else:
            raise ValueError(f"Unsupported fact-checking website: {url}.")
    except Exception as e:
        log_print(f"Error scraping article {url}: {str(e)}.")
        raise
...
def scrape_politifact_article(url):
    ...
    article_parts['verdict'] = match.group(1)
    article_parts['date'] = statement_date.get_text(strip=True)
    article_parts['claim'] = statement_quote.get_text(strip=True)
    article_parts['rationale'] = parse_full_text(full_text_html)
    article_parts['iytis'] = " ".join(iytis_html.get_text(separator=' ', strip=True).split())
    article_parts['url'] = url
    article_parts['fact_checker'] = 'PolitiFact'
