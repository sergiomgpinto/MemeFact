...
def _parse_update_fields(meme_post_url, disclaimer_post_url):
    meme_post_data = scrape_x_page(meme_post_url)['tweet']
    disclaimer_post_data = scrape_x_page(disclaimer_post_url)['tweet']
    views_count = meme_post_data['views_count'] + disclaimer_post_data['views_count']
    likes_count = meme_post_data['favorite_count'] + disclaimer_post_data['favorite_count']
    retweets_count = meme_post_data['retweet_count'] + disclaimer_post_data['retweet_count']
    quotes_count = meme_post_data['quote_count'] + disclaimer_post_data['quote_count']
    replies_count = meme_post_data['reply_count'] + disclaimer_post_data['reply_count'] - 1  # -1 because
...
def update_1h_fields(article_url, meme_post_url, disclaimer_post_url):
    ...
    metrics, engagement = _parse_update_fields(meme_post_url, disclaimer_post_url)
    update_meme_data('x_bot_data.csv', article_url, '1h', metrics, engagement)
...
def update_24h_fields(article_url, meme_post_url, disclaimer_post_url):
    ...
    metrics, engagement = _parse_update_fields(meme_post_url, disclaimer_post_url)
    update_meme_data('x_bot_data.csv', article_url, '24h', metrics, engagement)
...
def update_7d_fields(article_url, meme_post_url, disclaimer_post_url):
    ...
    metrics, engagement = _parse_update_fields(meme_post_url, disclaimer_post_url)
    update_meme_data('x_bot_data.csv', article_url, '7d', metrics, engagement)
...
def update_1m_fields(article_url, meme_post_url, disclaimer_post_url):
    ...
    metrics, engagement = _parse_update_fields(meme_post_url, disclaimer_post_url)
    update_meme_data('x_bot_data.csv', article_url, '1m', metrics, engagement)
...
def update_3m_fields(article_url, meme_post_url, disclaimer_post_url):
    ...
    metrics, engagement = _parse_update_fields(meme_post_url, disclaimer_post_url)
    update_meme_data('x_bot_data.csv', article_url, '3m', metrics, engagement)


def parse_virality_dataset_entry(meme: Meme, cached_x_data, results, article_parts, bot_profile_base_url):
    article_x_post_data = cached_x_data
    ...
    original_misinformation_post_url = article_parts['x_sources'][0]

    fact_check_article_url = article_parts['article_link']
    ...
    add_meme_entry('x_bot_data.csv',
                   [
                       fact_check_timestamp,  # timestamp
                       meme_post_url,  # meme post url
                       disclaimer_post_url,  # disclaimer url
                       original_misinformation_post_url,  # original misinformation post url
...
def create_meme_virality_dataset(filename):
    columns = [
        "timestamp",  # When the fact-check meme was posted
...
def update_meme_data(filename, article_url, time_point, metrics, engagement: Optional[int] = 0):
    root_path = get_git_root()
    file_path = root_path / 'data' / 'bot' / 'dataset' / filename
